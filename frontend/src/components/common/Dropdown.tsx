      ...
      {createPortal(
        <AnimatePresence>
          ...
          <motion.ul
            ...
            className={cn(
              'fixed z-[9999] py-1 bg-gray-900/95 backdrop-blur-xl border border-white/10 rounded-lg shadow-2xl shadow-black/30'
            )}
            style={{
              top: position.top,
              left: position.left,
          ...
          >
            {items.map((item, index) =>
              item.divider ? (
                <li key={`divider-${index}`} className="my-1 border-t border-white/10" />
              ) : (
...
export function DropdownTrigger({
  ...
  return (
    <button
      className={cn(
        'inline-flex h-10 items-center gap-2 px-3 py-2 text-sm text-white/70 bg-white/5 border border-white/10 rounded-lg',
        'hover:-translate-y-0.5 hover:bg-white/10 hover:border-white/20 transition-all',
        className
      )}
      {...props}
    >
